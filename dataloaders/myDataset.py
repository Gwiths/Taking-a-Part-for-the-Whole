...
class MyDataset(BaseDataset):
    def __init__(self, args, split='train', sample_mode='list', **kwargs):
        ...
        self.split = split
        if split == 'train':
            args.train_list = os.path.join(args.list_dir, args.train_csv + '.csv')
            self.transform = self.transform_train()
            _data_list = args.train_list
        elif split == 'val':
            args.val_list = os.path.join(args.list_dir, args.val_csv + '.csv')
            self.transform = self.transform_validation()
            _data_list = args.val_list
        elif split == 'test':
            args.test_list = os.path.join(args.list_dir, args.test_csv + '.csv')
            self.transform = self.transform_validation()
        ...
        if sample_mode == 'list' and (split == 'val' or split == 'test'):
            self.triplet_list = []
            '''
                The model validation process is carried out under the 1:2 matching task of v2f.
            '''
            with open(osp.join(self.args.list_dir, self.args.get('v2f_triplet_%s' % split) + '.txt'), 'r') as f:
    ...
    def load(self, metas, image_id=-1, audio_id=-1, with_filename=False):
        ...
        if self.split == 'train':
            ...
            if image_id < 0:
                ...
                image_id_aux = np.random.choice(range(len(metas['image_list'])))
                while image_id == self.sbs2[sample['ID']][0] or image_id == self.sbs[sample['ID']][0]:
                    image_id = np.random.choice(range(len(metas['image_list'])))
                while image_id_aux == self.sbs2[sample['ID']][0] or image_id_aux == self.sbs[sample['ID']][0]:
                    image_id_aux = np.random.choice(range(len(metas['image_list'])))
                self.sbs2[sample['ID']][0] = image_id
                self.sbs[sample['ID']][0] = image_id_aux

            if audio_id < 0:
                audio_id = np.random.choice(range(len(metas['audio_list'])))
                audio_id_aux = np.random.choice(range(len(metas['audio_list'])))
                while audio_id == self.sbs2[sample['ID']][1] or audio_id == self.sbs[sample['ID']][1]:
                    audio_id = np.random.choice(range(len(metas['audio_list'])))
                while audio_id_aux == self.sbs2[sample['ID']][1] or audio_id_aux == self.sbs[sample['ID']][1]:
                    audio_id_aux = np.random.choice(range(len(metas['audio_list'])))
                self.sbs2[sample['ID']][1] = audio_id
                self.sbs[sample['ID']][1] = audio_id_aux

            if self.modal == 'image' or self.modal == 'both':
                sample['image'] = self.load_image(metas, image_id)
                sample['image_aux'] = self.load_image(metas, image_id_aux)
                if with_filename:
            ...
            if self.modal == 'audio' or self.modal == 'both':
                ...
                sample['audio_aux'] = self.load_audio(metas, audio_id_aux)
                if with_filename:
