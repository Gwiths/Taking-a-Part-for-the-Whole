...
class Tester(BaseContainer):
    ...
    def load_feat(self):
        save_path = self.args.testing.extracted_features_path
    ...
    def test(self):
        ...
        all_res = []
        for l in args.test_lists:
            with open(osp.join(args.list_dir, l + '.txt'), 'r') as f:
                content = f.read().splitlines()
            content_lines = [i.split(' ') for i in content]
            if l.startswith('match'):
                res = self.performMatching(content_lines)
            elif l.startswith('verify'):
                res = self.performVerification(content_lines)
            elif l.startswith('reterival'):
                res = self.performRetrieval(content_lines)
            print('%s: %.6f'%(l, res))
            all_res.append(res)
    ...
    def performMatching(self, eval_list):
        N = len(eval_list[0]) - 1
        cnt_pos = 0
        cnt_neg = 0
    ...
    def performVerification(self, eval_list):
        pred = []
        target = []
    ...
    def performRetrieval(self, eval_list):
